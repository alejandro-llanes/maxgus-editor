...
| `gui-scroll-indicator` | bool | `#true` | A thin bar at a window's right edge while it scrolls, its place and length saying where in the buffer the window is and how much of it shows. It fades out once the window has been still for a moment, and never appears for a window showing the whole buffer. |
| `gui-prose-font` | string | `"sans-serif"` | The family the window sets prose in: what the language server says about a symbol, which the window draws in a reading face, wrapped to the pixel, rather than on the grid. `sans-serif` is whatever the system has set up as its sans-serif; a family the system lacks falls back to the code font. Ignored by the terminal. |
| `gui-title-format` | string | `"%m%b — %p — %a"` | What the window's title says. `%b` is the buffer's name, `%f` the file's path with the home directory as `~` (the buffer's name when there is no file), `%p` the project's name, `%P` its path, `%m` a `•` when the buffer is modified and nothing when it is not, `%a` the program's name, `%%` a per-cent sign. Ignored by the terminal. |
